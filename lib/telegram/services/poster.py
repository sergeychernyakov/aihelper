...
class Poster:
    ...
    async def send_to_all(self, post: Post, user_ids: Optional[List[int]] = None):
        ...
        try:
            # Determine which user IDs to send the message to
            if user_ids is None:
                # Fetch all user IDs if no specific user_ids are provided
                conversations = session.query(Conversation.user_id).distinct().all()
                user_ids = [conv.user_id for conv in conversations]

            # Send the post to the specified users
